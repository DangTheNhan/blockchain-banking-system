...
- [x] **Day 5: Deposit & Maturity Withdrawals**
  - Implement `openDeposit()` with APR/Penalty snapshotting.
  - Implement `withdrawAtMaturity()` using simple interest math.
- [x] **Day 6: Early Withdrawal & Renewals**
  - Implement `earlyWithdraw()` with penalty deduction.
  - Implement `renewDeposit()` (Manual) and `autoRenewDeposit()` (Bot).
- [x] **Day 7: Logic Integration & Refactoring**
  - Add `ReentrancyGuard` to `SavingCore` for security.
  - Create `scripts/deploy.ts` to deploy and link all contracts on Localhost.
...
## 🛡️ Phase 4: Comprehensive Testing & Bonus Challenges (Days 11 - 13)
*Note: Moved back to prioritize Frontend Demo.*
- [ ] **Day 11: Happy Path Testing**
  - Use Hardhat time helpers to simulate maturity and grace periods.
  - Ensure solidity-coverage runs successfully.
- [ ] **Day 12: Edge Cases & Security Checks**
  - Test double withdrawals, empty vault, boundary timestamps (`>=` vs `>`).
- [ ] **Day 13: Bonus Challenges Integration**
